            ...
            <div class="layui-form toolbar">
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <button class="layui-btn layui-btn-sm layui-btn-danger" onclick="Admin.del('<?=links('app','task_list_del')?>','pay')"><i class="layui-icon"></i>Xóa hàng loạt</button>
                    </div>
                    ...
                    <div class="layui-inline mr0">
                        <div class="layui-input-inline">
                            <input type="text" name="uid" placeholder="Nhập ID Người dùng" autocomplete="off" class="layui-input h30" value="">
                        </div>
                    </div>
                    <div class="layui-inline mr0">
            ...
            <table class="layui-table" lay-even lay-skin="row" lay-data="{url:'<?=links('app','task_list_ajax')?>',limit:20,limits:[20,30,50,100,500],page:{layout:['count','prev','page','next','refresh','skip','limit']},id:'pay'}" lay-filter="pay">
              <thead>
                ...
                <?php if(defined('MOBILE')){ ?>
                    ...
                    <th lay-data="{field:'tname',align:'center'}">Tiêu đề Nhiệm vụ</th>
                    <th lay-data="{field:'cion',align:'center'}">Phần thưởng <?=Pay_Cion_Name?></th>
                    <th lay-data="{field:'vip',align:'center'}">Phần thưởng VIP</th>
                    <th lay-data="{align:'center',templet:'#cmdTpl'}">Hoạt động</th>
                <?php }else{ ?>
                    <th lay-data="{field:'id',type:'checkbox',width:60,align:'center'}"></th>
                    <th lay-data="{field:'tname',align:'center'}">Tiêu đề Nhiệm vụ</th>
                    <th lay-data="{field:'uid',align:'center'}">ID Người nhận thưởng</th>
                    <th lay-data="{field:'cion',align:'center'}">Phần thưởng <?=Pay_Cion_Name?></th>
                    <th lay-data="{field:'vip',align:'center'}">Phần thưởng VIP</th>
...
<script type="text/html" id="cmdTpl">
    <button style="margin-left:5px;" title="Xóa" class="layui-btn-danger layui-btn layui-btn-xs" onclick="Admin.del('<?=links('app','task_list_del')?>','{{d.id}}',this)" href="javascript:;" ><i class="layui-icon">&#xe640;</i>Xóa</button>
</script>
</body>
</html>
